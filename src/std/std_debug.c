...
void std_register_debug(VM* vm) {
    ...
    DenseObject* objVm = dense_object_create_with(vm, 3 STD_DEBUG_ENTRY(acc, vm_acc)STD_DEBUG_ENTRY(heapSize, vm_heap_size)STD_DEBUG_ENTRY(stackSize, vm_stack_size));

    DenseObject* obj = dense_object_create_with(vm, 2,
                                                "vm", sizeof("vm") - 1, DENSE_VALUE((DenseValue *) objVm)STD_DEBUG_ENTRY(type, type));
